...
# ============================================================================
# 3.1 УНИКАЛИЗАЦИЯ ЭМОДЗИ-МАСКИ ЗАГОЛОВКОВ (индивидуальный стиль магазина)
# ============================================================================
#
# Финальная архитектура (исправляет ВСЕ выявленные регрессии):
#   * Таблица EMOJI_ROTATION строится по инвентарю донорских эмодзи из
#     fields[summary][ru]/[en] и является СТРОГОЙ ИНЪЕКЦИЕЙ: каждый донорский
#     символ отображается в РАЗНЫЙ фирменный символ, которого нет у донора.
#     Ротация выполняется на УРОВНЕ КЛАСТЕРОВ подряд идущих маркеров — это
#     гарантирует, что ни один символ текста не удаляется и не склеивается
#     (баги «Firs Light» и «XboxStandardБезопасно» исчезают по построению).
#   * Кластеры символов, НЕ входящих в таблицу (региональные индикаторы
#     флагов 🇹🇷/🇷🇺, ZWJ-составные), сохраняются как есть; одиночные флаги
#     вырезаются только вместе с соседним текстом-«скобкой» целиком (см.
#     _strip_flag_pairs) — буквы при этом не трогаются.
#   * Символы маски НИКОГДА не вырезаются из остаточного текста заголовка
#     (старый шаг «_should_strip» жевал текст) — вместо этого незакрытые
...
DONOR_EMOJIS = set()  # заполняется в main() из инвентаря фактического дампа
...
MANUAL_ROTATION = {
    ...
    "❤️": "💗", "❤": "💗",   # красное сердце -> растущее сердце
    "⚫️": "◾", "⚫": "◾",   # черный круг -> черный малый квадрат
    "🌠": "🧊",   # падающая звезда -> лед
    "🥰": "😊",   # влюбленное лицо -> улыбка
    "🗣️": "🎮", "🗣": "🎮",   # говорящая голова -> геймпад
    "☠️": "💀", "☠": "💀",   # череп -> череп с костями (консистентно)
    "🌫️": "🌬️", "🌫": "🌬️",   # туман -> дыхание ветра
    "🤖": "👤",   # робот -> силуэт (ручная выдача!)
    "💠": "📿",   # артефакт ТЗ-маппинга -> четки (🛎 уже занят им выше)
    "▪️": "◾",   # артефакт ТЗ-маппинга -> малый квадрат
    "🇹🇷": "", "🇷🇺": "", "🇺🇦": "", "🇰🇿": "",  # флаговые пары вырезаются
}
...
def build_emoji_rotation(donor_emojis):
    ...
    Гарантии:
      ...
      * разные донорские символы получают РАЗНЫЕ цели (инъективность);
      * флаговые последовательности отображаются в '' (вырезаются)."""
...
def _cluster_base(txt):
    return "".join(ch for ch in txt if ch not in ("\uFE0F", "\u200D"))


def _rotate_clusters(s):
    """Кластерная ротация: каждый кластер подряд идущих маркеров заменяется
    склейкой целей его символов (FE0F/ZWJ приклеиваются к предыдущему).
    Инъективность таблицы => склейка уникальна, коллизий быть не может.
    Кластеры, содержащие символы вне таблицы (например, региональные
    индикаторы флагов), обрабатываются посимвольно: известные заменяются,
    неизвестные сохраняются как есть — ТЕКСТ НИКОГДА НЕ УДАЛЯЕТСЯ."""
    ...
    while i < n:
        j = i
        while j + 1 < n and parts[j + 1] == "":
            j += 1                       # кластер marks[i..j] без текста между
        repl = []
        for k in range(i, j + 1):
            m = marks[k]
            if m in ("\uFE0F", "\u200D"):
                continue                 # приклеен к цели предыдущего символа
            if m in EMOJI_ROTATION:
                repl.append(EMOJI_ROTATION[m])
            else:
                repl.append(m)           # неизвестный символ сохраняем
        out.append(parts[i])             # текст ПЕРЕД первым маркером кластера
        out.append("".join(repl))
        i = j + 1
    out.append(parts[-1])
    return "".join(out)


def _strip_flag_pairs(s):
    """Вырезание целых флаговых последовательностей (пар региональных
    индикаторов) — только сами флаги, соседний текст не затрагивается."""
    return _REGIONAL_INDICATOR_RE.sub("", s)
...
def uniquify_summary_emojis(s):
    ...
    # (❗❗❗...❗❗❗), имеют пару и не трогаются.
    donor_bases = {e.replace("\uFE0F", "") for e in DONOR_EMOJIS}
    clusters = [(m.start(), m.end(), m.group()) for m in _CLUSTER_RE.finditer(s)]
    add_at = {}
    consumed = set()
    for idx, (a, b, txt) in enumerate(clusters):
        if idx in consumed:
            continue
        base = _cluster_base(txt)
        # интересуют только кластеры, целиком состоящие из донорских символов
        if not base or not all(ch in donor_bases for ch in base):
            continue
        after = s[b:]
        before = s[:a]
        if not after:                    # конец строки — закрывать нечего
            continue
        paired = None
        for jdx in range(idx + 1, len(clusters)):
            if jdx in consumed:
                continue
            a2, b2, t2 = clusters[jdx]
            if _cluster_base(t2) == base and s[a2 - 1:a2].strip():
                paired = jdx
                break
        if paired is None:
            # открывающая скобка: перед ней начало строки или текст
    ...
    if add_at:
        pieces = []
        last = 0
        for p in sorted(add_at):
            pieces.append(s[last:p])
            pieces.append(add_at[p])
            last = p
